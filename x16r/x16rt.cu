/**
 * X16RT algorithm (X16 with Randomized chain order)
 *
...
static void x16rt_getTimeHash( const uint32_t timeStamp, void* timeHash )
{
...
static void x16rt_getAlgoString( const uint32_t *timeHash, char *output)
{
   char *sptr = output;
...
extern "C" void x16rt_hash(void *output, const void *input)
{
   ...
   x16rt_getTimeHash( in32[17], &timeHash );
