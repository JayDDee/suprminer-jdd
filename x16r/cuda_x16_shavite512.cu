...
#define TPB 128
__device__ __align__(64) uint32_t d_AES0[256] = {
...
__device__ __align__(64) uint32_t d_AES3[256] = {
...
__device__ __forceinline__
static void c512_80(const uint32_t* sharedMemory, uint32_t *state, uint32_t *msg, const uint32_t count)
{
	uint32_t p0, p1, p2, p3, p4, p5, p6, p7;
	uint32_t p8, p9, pA, pB, pC, pD, pE, pF;
	uint32_t x0, x1, x2, x3;
...
//		sharedMemory[(threadIdx.x << 1) + 1] = temp.y;
		sharedMemory[256+(threadIdx.x << 1) + 0] = (temp.x);
		sharedMemory[256 + (threadIdx.x << 1) + 1] = (temp.y);
		sharedMemory[512 + (threadIdx.x << 1) + 0] = ROL8(temp.x);
		sharedMemory[512 + (threadIdx.x << 1) + 1] = ROL8(temp.y);
		sharedMemory[768 + (threadIdx.x << 1) + 0] = ROL16(temp.x);
		sharedMemory[768 + (threadIdx.x << 1) + 1] = ROL16(temp.y);
	}
}

__global__ __launch_bounds__(TPB, 5)
void x16_shavite512_gpu_hash_80(uint32_t threads, uint32_t startNounce, void *outputHash)
{
...
void x16_shavite512_cpu_hash_80(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_outputHash, int order)
{
	...
	x16_shavite512_gpu_hash_80<<<grid, block>>>(threads, startNounce, d_outputHash);
}

__host__
void x16_shavite512_cpu_init(int thr_id, uint32_t threads)
{
//	aes_cpu_init(thr_id);
}
