...
int opt_api_port = 0;  // 4068 is the usual port
...
int main(int argc, char *argv[])
{
	...
	printf("*** suprminer-jdd " PACKAGE_VERSION " for nVidia GPUs ***\n\n");
        printf("Forked from spmod/sp-suprminer, forked from ocminer,\n");
        printf("based on ccminer by tpruvot@github. Originally based on\n");
        printf("Christian Buchner and Christian H. project. Includes work\n");
        printf("from sp, alexis78, djm34, djEzo, tsiv, krnlx and JayDDee.\n\n");

	if (!opt_quiet) {
		const char* arch = is_x64() ? "64-bits" : "32-bits";
#ifdef _MSC_VER
		printf("Built on __DATE__ with VC++ %d and nVidia CUDA SDK %d.%d %s\n\n", msver(),
                        CUDART_VERSION/1000, (CUDART_VERSION % 1000)/10, arch);
#else
		printf("Built for %s on " __DATE__ " with GCC %d.%d.%d & nVidia Cuda %d.%d\n\n",
