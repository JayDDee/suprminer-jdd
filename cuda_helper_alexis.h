...
__device__ __forceinline__
uint32_t or3( uint32_t a,uint32_t b,uint32_t c )
{
    uint32_t result;
    #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
       asm ("lop3.b32 %0, %1, %2, %3, 0xFE;" : "=r"(result) : "r"(a), "r"(b),"r"(c));
    #else
       result = a | b | c;
    #endif
    return result;
...
__device__ __forceinline__
uint2 or3( const uint2 a,const uint2 b,const uint2 c )
{
   uint2 result;
   #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
      asm ("lop3.b32 %0, %1, %2, %3, 0xFE;" : "=r"(result.x) : "r"(a.x), "r"(b.x),"r"(c.x));
      asm ("lop3.b32 %0, %1, %2, %3, 0xFE;" : "=r"(result.y) : "r"(a.y), "r"(b.y),"r"(c.y)); 
   #else
      result = a | b | c;
   #endif
   return result;
...
__device__ __forceinline__
uint32_t xorand( uint32_t a,uint32_t b,uint32_t c )
{
    uint32_t result;
    #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
       asm ("lop3.b32 %0, %1, %2, %3, 0x78;" : "=r"(result) : "r"(a), "r"(b),"r"(c));
    #else
       result = a ^ ( b & c );
    #endif
    return result;
...
__device__ __forceinline__
uint2 xorand( const uint2 a,const uint2 b,const uint2 c )
{
    uint2 result;
    #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
        asm ("lop3.b32 %0, %1, %2, %3, 0x78;" : "=r"(result.x) : "r"(a.x), "r"(b.x),"r"(c.x));
        asm ("lop3.b32 %0, %1, %2, %3, 0x78;" : "=r"(result.y) : "r"(a.y), "r"(b.y),"r"(c.y));
    #else
        result = a ^ ( b & c );
    #endif
    return result;
...
__device__ __forceinline__
uint32_t andxor( uint32_t a,uint32_t b,uint32_t c )
{
    uint32_t result;
    #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
       asm ("lop3.b32 %0, %1, %2, %3, 0x60;" : "=r"(result) : "r"(a), "r"(b),"r"(c));
    #else
       result = a & ( b ^ c );
    #endif
    return result;
...
__device__ __forceinline__
uint2 andxor( const uint2 a,const uint2 b,const uint2 c )
{
    uint2 result;
    #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
        asm ("lop3.b32 %0, %1, %2, %3, 0x60;" : "=r"(result.x) : "r"(a.x), "r"(b.x),"r"(c.x));
        asm ("lop3.b32 %0, %1, %2, %3, 0x60;" : "=r"(result.y) : "r"(a.y), "r"(b.y),"r"(c.y));
    #else
        result = a & ( b ^ c );
    #endif
    return result;
...
__device__ __forceinline__
uint32_t xoror( uint32_t a,uint32_t b,uint32_t c )
{
    uint32_t result;
    #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
       asm ("lop3.b32 %0, %1, %2, %3, 0x1e;" : "=r"(result) : "r"(a), "r"(b),"r"(c));
    #else
       result = a ^ ( b | c );
    #endif
    return result;
...
__device__ __forceinline__
uint2 xoror( const uint2 a,const uint2 b,const uint2 c )
{
    uint2 result;
    #if __CUDA_ARCH__ >= 500 && CUDA_VERSION >= 7050
        asm ("lop3.b32 %0, %1, %2, %3, 0x1e;" : "=r"(result.x) : "r"(a.x), "r"(b.x),"r"(c.x));
        asm ("lop3.b32 %0, %1, %2, %3, 0x1e;" : "=r"(result.y) : "r"(a.y), "r"(b.y),"r"(c.y));
    #else
        result = a ^ ( b | c );
    #endif
    return result;
