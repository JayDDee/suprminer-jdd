...
__device__ __forceinline__ void G256_AddRoundConstantQ_quad(uint32_t &x7, uint32_t &x6, uint32_t &x5, uint32_t &x4, uint32_t &x3, uint32_t &x2, uint32_t &x1, uint32_t &x0, const int round)
{
	...
	asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x0) : "r"(x0), "r"(-(round & 0x01)), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)
	asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x1) : "r"(x1), "r"(-(round & 0x02)), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)
	asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x2) : "r"(x2), "r"(-(round & 0x04)), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)
	asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x3) : "r"(x3), "r"(-(round & 0x08)), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)

        asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x4) : "r"(x4), "r"(0xAAAA0000), "r"(andmask1)); 
        asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x5) : "r"(x5), "r"(0xCCCC0000), "r"(andmask1)); 
        asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x6) : "r"(x6), "r"(0xF0F00000), "r"(andmask1)); 
        asm("lop3.b32 %0, %1, %2, %3, 0x87;" : "=r"(x7) : "r"(x7), "r"(0xFF000000), "r"(andmask1)); 
}
...
__device__ __forceinline__ void G256_AddRoundConstantP_quad(uint32_t &x7, uint32_t &x6, uint32_t &x5, uint32_t &x4, uint32_t &x3, uint32_t &x2, uint32_t &x1, uint32_t &x0, const int round)
{
	...
	asm("lop3.b32 %0, %1, %2, %3, 0x78;" : "=r"(x0) : "r"(x0), "r"(-(round & 0x01)), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)
	asm("lop3.b32 %0, %1, %2, %3, 0x78;" : "=r"(x1) : "r"(x1), "r"(-(round & 0x02) >> 1), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)
	asm("lop3.b32 %0, %1, %2, %3, 0x78;" : "=r"(x2) : "r"(x2), "r"(-(round & 0x04) >> 2), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)
	asm("lop3.b32 %0, %1, %2, %3, 0x78;" : "=r"(x3) : "r"(x3), "r"(-(round & 0x08) >> 3), "r"(andmask1)); //0x87= ~0xF0 ^ (( 0xCC & 0xAA)

	x4 = xorand( x4, 0xAAAA, andmask1 );
        x5 = xorand( x5, 0xCCCC, andmask1 );
        x6 = xorand( x6, 0xF0F0, andmask1 );
        x7 = xorand( x7, 0xFF00, andmask1 );
}
...
static void G16mul_quad( uint32_t &x3, uint32_t &x2, uint32_t &x1, uint32_t &x0,const uint32_t &y3, const uint32_t &y2, const uint32_t &y1, const uint32_t &y0 )
{
    ...
    t0 = xor3x( x2, x0, (x3 ^ x1) ) & xor3x( y2, y0, (y3 ^ y1) );
    t1 = xorand( t0, (x2 ^ x0), (y2 ^y0) );
    t2 = xor3x( ((x3 ^ x1) & (y3 ^ y1)), t0, t1 );
    t0 = andxor( (x2 ^ x3), y2, y3 );
    x3 = xor3x( (x3 & y3), t1, t0 );
    x2 = xor3x( (x2 & y2), t2, t0 );
    t0 = andxor( (x0 ^ x1), y0, y1 );
    x1 = xor3x( (x1 & y1), t1, t0 );
    x0 = xor3x( (x0 & y0), t2, t0 );
}
...
static void G256_inv_quad( uint32_t &x7, uint32_t &x6, uint32_t &x5, uint32_t &x4, uint32_t &x3, uint32_t &x2, uint32_t &x1, uint32_t &x0 )
{
    ...
    a = xor3x( t4, t3, t1 );
    t5 = xorand( a, t3, t1 );
    asm( "lop3.b32 %0, %1, %2, %3, 0x56;" : "=r"(t6) : "r"(t2), "r"(t0), "r"(a) );
//    t6 = xor3x( (t2 & t0), a, (t2 ^ t0) );
    t4 = andxor( b, t5, t6 );
    t1 = xorand( t4, t6, t1 );
    t0 = xorand( t4, t5, t0 );
    t4 = andxor( ( t5 ^ t6 ), t2, t3 );
    t3 = xorand( t4, t6, t3 );
    t2 = xorand( t4, t5, t2 );
...
#define S(idx, l)	__byte_perm(A(idx,l),A(idx,l+1),0x5432)
	...
	uint32_t b[8];
        ...
        const uint32_t tmp = xor3x( S(7, 1), S(7, 3), A(7, 2) ) ^ A(7, 3);

        b[6] = xor3x( S(6, 1), S(6, 3), A(6, 2) ) ^ A(6, 3);

        const uint32_t tmp2= xor3x( b[6], S(7, 3), A(7, 4) ) ^ xor3x( S(7, 4), A(7, 5), S(7, 6) );

        b[5] = xor3x( S(5, 1), S(5, 3), A(5, 2) ) ^ A(5, 3);
        b[6] = xor3x( b[5], S(6, 3), A(6, 4) ) ^ xor3x( S(6, 4), A(6, 5), S(6, 6) );
        r[7] = xor3x( b[6], S(7, 2), A(7, 2) ) ^ xor3x( S(7, 3), A(7, 3), A(7, 5) );

        b[4] = xor3x( S(4, 1), S(4, 3), A(4, 2) ) ^ A(4, 3);
        b[5] = xor3x( b[4], S(5, 3), A(5, 4) ) ^ xor3x( S(5, 4), A(5, 5), S(5, 6) );
        r[6] = xor3x( b[5], S(6, 2), A(6, 2) ) ^ xor3x( S(6, 3), A(6, 3), A(6, 5) );

        b[3] = xor3x( S(3, 1), S(3, 3), A(3, 2) ) ^ A(3, 3);
        b[4] = xor3x( xor3x( b[3], S(4, 3), A(4, 4) ), xor3x( S(4, 4), A(4, 5), S(4, 6) ), tmp );
        r[5] = xor3x( b[4], S(5, 2), A(5, 2) ) ^ xor3x( S(5, 3), A(5, 3), A(5, 5) );

        b[2] = xor3x( S(2, 1), S(2, 3), A(2, 2) ) ^ A(2, 3);
        b[3] = xor3x( xor3x( b[2], S(3, 3), A(3, 4) ), xor3x( S(3, 4), A(3, 5), S(3, 6) ), tmp );
        r[4] = xor3x( xor3x( b[3], S(4, 2), A(4, 2) ), xor3x( S(4, 3), A(4, 3), A(4, 5) ), tmp2 );

        b[1] = xor3x ( S(1, 1), S(1, 3), A(1, 2) ) ^ A(1, 3);
        b[2] = xor3x( b[1], S(2, 3), A(2, 4) ) ^ xor3x( S(2, 4), A(2, 5), S(2, 6) );
        r[3] = xor3x( xor3x( b[2], S(3, 2), A(3, 2) ), xor3x( S(3, 3), A(3, 3), A(3, 5) ), tmp2 );

        b[0] = xor3x( S(0, 1), S(0, 3), A(0, 2) ) ^ A(0, 3);
        b[1] = xor3x( xor3x( b[0], S(1, 3), A(1, 4) ), xor3x( S(1, 4), A(1, 5), S(1, 6) ), tmp );
        r[2] = xor3x( b[1], S(2, 2), A(2, 2) ) ^ xor3x( S(2, 3), A(2, 3), A(2, 5) );

        b[0] = xor3x( tmp, S(0, 3), A(0, 4) ) ^ xor3x( S(0, 4), A(0, 5), S(0, 6) );
        r[1] = xor3x( xor3x( b[0], S(1, 2), A(1, 2) ), xor3x( S(1, 3), A(1, 3), A(1, 5) ), tmp2 );

        r[0] = xor3x( tmp2, S(0, 2), A(0, 2) ) ^ xor3x( S(0, 3), A(0, 3), A(0, 5) );
...
__device__ __forceinline__
static void groestl512_perm_P_quad_1(uint32_t *const __restrict__ r)
{
...
/*#pragma unroll
	for (int round = 10; round<14; round++)
	{
		G256_AddRoundConstantP_quad(r[7], r[6], r[5], r[4], r[3], r[2], r[1], r[0], round);
		sbox_quad(r);
		G256_ShiftBytesP_quad(r[7], r[6], r[5], r[4], r[3], r[2], r[1], r[0]);
...
static void groestl512_perm_P_quad(uint32_t *const __restrict__ r)
{
	...
	#pragma unroll
	for (int round = 13; round<14; round++)
	{
		G256_AddRoundConstantP_quad(r[7], r[6], r[5], r[4], r[3], r[2], r[1], r[0], round);
		sbox_quad(r);
		G256_ShiftBytesP_quad(r[7], r[6], r[5], r[4], r[3], r[2], r[1], r[0]);
...
/*	#pragma unroll
	for (int round = 8; round<14; round++)
	{
		G256_AddRoundConstantQ_quad(r[7], r[6], r[5], r[4], r[3], r[2], r[1], r[0], round);
		sbox_quad(r);
		G256_ShiftBytesQ_quad(r[7], r[6], r[5], r[4], r[3], r[2], r[1], r[0]);
...
__device__ __forceinline__
void groestl512_progressMessage_quad(uint32_t *const __restrict__ state, uint32_t *const __restrict__ message, const uint32_t thr){
