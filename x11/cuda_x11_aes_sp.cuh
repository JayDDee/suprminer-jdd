...
void aes_gpu_init64(uint32_t sharedMemory[4][256])
{
	/* each thread startup will fill 2 uint32 */
	uint2 temp = __ldg(&((uint2*)&d_AES0)[threadIdx.x]);
	uint2 temp2 = __ldg(&((uint2*)&d_AES0)[threadIdx.x + 128]);
	...
	if (threadIdx.x < 64)
	{
		...
		sharedMemory[0][(threadIdx.x << 1) + 0] = temp2.x;
		sharedMemory[0][128 + (threadIdx.x << 1) + 1] = temp2.y;
		sharedMemory[1][128 + (threadIdx.x << 1) + 0] = ROL8(temp2.x);
		sharedMemory[1][128 + (threadIdx.x << 1) + 1] = ROL8(temp2.y);
		sharedMemory[2][128 + (threadIdx.x << 1) + 0] = ROL16(temp2.x);
		sharedMemory[2][128 + (threadIdx.x << 1) + 1] = ROL16(temp2.y);
		sharedMemory[3][128 + (threadIdx.x << 1) + 0] = ROR8(temp2.x);
		sharedMemory[3][128 + (threadIdx.x << 1) + 1] = ROR8(temp2.y);
	}
}


__device__ __forceinline__
void aes_gpu_init32(uint32_t sharedMemory[4][256])
{
	if (threadIdx.x < 32)
	{
		...
		/* each thread startup will fill 2 uint32 */
		uint2 temp = __ldg(&((uint2*)&d_AES0)[threadIdx.x]);
		uint2 temp1 = __ldg(&((uint2*)&d_AES0)[threadIdx.x + 32]);
		uint2 temp2 = __ldg(&((uint2*)&d_AES0)[threadIdx.x + 64]);
		uint2 temp3 = __ldg(&((uint2*)&d_AES0)[threadIdx.x + 96]);
		...
		sharedMemory[0][32 + (threadIdx.x << 1) + 0] = temp1.x;
		sharedMemory[0][32 + (threadIdx.x << 1) + 1] = temp1.y;
		sharedMemory[1][32 + (threadIdx.x << 1) + 0] = ROL8(temp1.x);
		sharedMemory[1][32 + (threadIdx.x << 1) + 1] = ROL8(temp1.y);
		sharedMemory[2][32 + (threadIdx.x << 1) + 0] = ROL16(temp1.x);
		sharedMemory[2][32 + (threadIdx.x << 1) + 1] = ROL16(temp1.y);
		sharedMemory[3][32 + (threadIdx.x << 1) + 0] = ROR8(temp1.x);
		sharedMemory[3][32 + (threadIdx.x << 1) + 1] = ROR8(temp1.y);

		sharedMemory[0][64 + (threadIdx.x << 1) + 0] = temp2.x;
		sharedMemory[0][64 + (threadIdx.x << 1) + 1] = temp2.y;
		sharedMemory[1][64 + (threadIdx.x << 1) + 0] = ROL8(temp2.x);
		sharedMemory[1][64 + (threadIdx.x << 1) + 1] = ROL8(temp2.y);
		sharedMemory[2][64 + (threadIdx.x << 1) + 0] = ROL16(temp2.x);
		sharedMemory[2][64 + (threadIdx.x << 1) + 1] = ROL16(temp2.y);
		sharedMemory[3][64 + (threadIdx.x << 1) + 0] = ROR8(temp2.x);
		sharedMemory[3][64 + (threadIdx.x << 1) + 1] = ROR8(temp2.y);

		sharedMemory[0][96 + (threadIdx.x << 1) + 0] = temp3.x;
		sharedMemory[0][96 + (threadIdx.x << 1) + 1] = temp3.y;
		sharedMemory[1][96 + (threadIdx.x << 1) + 0] = ROL8(temp3.x);
		sharedMemory[1][96 + (threadIdx.x << 1) + 1] = ROL8(temp3.y);
		sharedMemory[2][96 + (threadIdx.x << 1) + 0] = ROL16(temp3.x);
		sharedMemory[2][96 + (threadIdx.x << 1) + 1] = ROL16(temp3.y);
		sharedMemory[3][96 + (threadIdx.x << 1) + 0] = ROR8(temp3.x);
		sharedMemory[3][96 + (threadIdx.x << 1) + 1] = ROR8(temp3.y);
	}
}
...
static void aes_round_LDG(const uint32_t sharedMemory[4][256], const uint32_t x0, const uint32_t x1, const uint32_t x2, const uint32_t x3, const uint32_t k0, uint32_t &y0, uint32_t &y1, uint32_t &y2, uint32_t &y3){
	...
	y3 ^= sharedMemory[2][__byte_perm(x1, 0, 0x4442)];
	y3 ^= __ldg(&d_AES3[__byte_perm(x2, 0, 0x4443)]);
	y3 ^= __ldg(&d_AES0[x3 & 0xff]);

	y2 = sharedMemory[2][__byte_perm(x0, 0, 0x4442)];
	y2 ^= __ldg(&d_AES3[__byte_perm(x1, 0, 0x4443)]);
	y2 ^= __ldg(&d_AES0[x2 & 0xff]);
	y2 ^= sharedMemory[1][__byte_perm(x3, 0, 0x4441)];
...
static void aes_round(const uint32_t sharedMemory[4][256], const uint32_t x0, const uint32_t x1, const uint32_t x2, const uint32_t x3, uint32_t &y0, uint32_t &y1, uint32_t &y2, uint32_t &y3)
{
	...
	y3 ^= sharedMemory[2][__byte_perm(x1, 0, 0x4442)];
	y3 ^= __ldg(&d_AES3[__byte_perm(x2, 0, 0x4443)]);
	y3 ^= sharedMemory[0][x3 & 0xff];
	y2 = sharedMemory[2][__byte_perm(x0, 0, 0x4442)];
	y2 ^= __ldg(&d_AES3[__byte_perm(x1, 0, 0x4443)]);
	y2 ^= sharedMemory[0][x2 & 0xff];
	y2 ^= sharedMemory[1][__byte_perm(x3, 0, 0x4441)];
	y1 = __ldg(&d_AES3[__byte_perm(x0, 0, 0x4443)]);
#ifdef INTENSIVE_GMF
	y1 ^= __ldg(&d_AES0[x1 & 0xff]);
#else
	y1 ^= sharedMemory[0][x1 & 0xff];
#endif
	y1 ^= sharedMemory[1][__byte_perm(x2, 0, 0x4441)];
...
__device__ __forceinline__  void AES_2ROUND(const uint32_t sharedMemory[4][256], uint32_t &x0, uint32_t &x1, uint32_t &x2, uint32_t &x3, uint32_t &k0)
{
...
__device__ __forceinline__  void AES_2ROUND_LDG2(const uint32_t sharedMemory[4][256], uint32_t &x0, uint32_t &x1, uint32_t &x2, uint32_t &x3, uint32_t &k0)
{
	uint32_t y0, y1, y2, y3;
...
{
	y0 = sharedMemory[(threadIdx.x & 31) + ((x0 & 0xff) << 5)];
	...
	y1 ^= sharedMemory[(threadIdx.x & 31) + ((x1 & 0xff) << 5)];
	...
	y2 ^= sharedMemory[(threadIdx.x & 31) + ((x2 & 0xff) << 5)];
	...
	y3 ^= sharedMemory[(threadIdx.x & 31) + ((x3 & 0xff) << 5)];
...
__device__ __forceinline__
static void AES_ROUND_NOKEY_32(const uint32_t sharedMemory[1024 * 8], uint32_t &x1, uint32_t &x2, uint32_t &x3, uint32_t &x4)
{
	uint32_t y0, y1, y2, y3;
	aes_round_32(sharedMemory, x1, x2, x3, x4, y0, y1, y2, y3);
...
__device__ __forceinline__
static void KEY_EXPAND_ELT_32(const uint32_t sharedMemory[1024 * 8], uint32_t &x1, uint32_t &x2, uint32_t &x3, uint32_t &x4)
{
...
__device__ __forceinline__
void aes_gpu_init128_32(uint32_t sharedMemory[1024 * 8])
{
	/* each thread startup will fill a uint32 */
	const uint32_t thread = threadIdx.x << 5;
	const uint32_t thread2 = ((threadIdx.x + 128) << 5);

	uint32_t temp = __ldg(&d_AES0[threadIdx.x]);
	uint32_t temp2 = __ldg(&d_AES0[threadIdx.x + 128]);
	sharedMemory[thread] = temp;
	sharedMemory[1 + thread] = temp;
	sharedMemory[2 + thread] = temp;
	sharedMemory[3 + thread] = temp;
	...
	sharedMemory[thread2] = temp2;
	sharedMemory[1 + thread2] = temp2;
	sharedMemory[2 + thread2] = temp2;
	sharedMemory[3 + thread2] = temp2;
	sharedMemory[4 + thread2] = temp2;
	sharedMemory[5 + thread2] = temp2;
	sharedMemory[6 + thread2] = temp2;
	sharedMemory[7 + thread2] = temp2;
	sharedMemory[8 + thread2] = temp2;
	sharedMemory[9 + thread2] = temp2;
	sharedMemory[10 + thread2] = temp2;
	sharedMemory[11 + thread2] = temp2;
	sharedMemory[12 + thread2] = temp2;
	sharedMemory[13 + thread2] = temp2;
	sharedMemory[14 + thread2] = temp2;
	sharedMemory[15 + thread2] = temp2;
	sharedMemory[16 + thread2] = temp2;
	sharedMemory[17 + thread2] = temp2;
	sharedMemory[18 + thread2] = temp2;
	sharedMemory[19 + thread2] = temp2;
	sharedMemory[20 + thread2] = temp2;
	sharedMemory[21 + thread2] = temp2;
	sharedMemory[22 + thread2] = temp2;
	sharedMemory[23 + thread2] = temp2;
	sharedMemory[24 + thread2] = temp2;
	sharedMemory[25 + thread2] = temp2;
	sharedMemory[26 + thread2] = temp2;
	sharedMemory[27 + thread2] = temp2;
	sharedMemory[28 + thread2] = temp2;
	sharedMemory[29 + thread2] = temp2;
	sharedMemory[30 + thread2] = temp2;
